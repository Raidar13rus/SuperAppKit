...
@interface NSArray<T>(KITEnumerable)<KITEnumerable>
...
- (NSArray<__kindof T> *)kit_filter:(BOOL (NS_NOESCAPE ^)(T obj))predicate;
...
@interface NSArray<T>(KITCollection)<KITCollection>
...
@interface NSMutableArray<T>(KITCollection)

- (_Nullable T)kit_moveObjectAtIndex:(NSUInteger)from toIndex:(NSUInteger)to;

- (_Nullable T)kit_addObject:(_Nullable T)object;
